VulpOS is a privacy and security focused Android OS for Google Pixel phones.

<b>This is renamed/customized fork of [RattlesnakeOS](https://github.com/dan-v/rattlesnakeos-stack)!</b><br />
It's mainly for personal and friend use. I would recommend starting with the official RattlesnakeOS if you are looking for something to use youreself.
...
## Background
VulpOS is truly just stock AOSP and has all of the baseline privacy and security features from there. Unlike other alternative Android OSes, it aims to keep security on par with stock Android by keeping critical security features like verified boot enabled, ensuring monthly OTA security updates not only update the OS but also the device specific drivers and firmware, and by not adding additional features or software that will needlessly increase attack surface. By not deviating from stock AOSP, updating to new major Android releases doesn't require any major porting effort and this means devices running VulpOS continue to receive proper security updates without delay.

## What is vulpos-stack?
Rather than providing random binaries of VulpOS to install on your phone, I've gone the route of creating a cross platform tool, `vulpos-stack`, that provisions a "stack", which is just all of the [AWS](https://aws.amazon.com/) cloud infrastructure needed to continuously build your own personal VulpOS, with your own signing keys, and your own OTA updates. It uses [AWS Lambda](https://aws.amazon.com/lambda/features/) to provision [EC2 spot instances](https://aws.amazon.com/ec2/spot/) that build VulpOS and upload artifacts to [S3](https://aws.amazon.com/s3/). Resulting OS builds are configured to receive over the air updates from this environment. It only costs a few dollars a month to run (see FAQ for detailed cost breakdown).
...
* On your local computer, using the CLI, generate an SSH key and upload the public key to all regions in AWS as shown below. By default we'll name the keypair `vulpos` in AWS. The public and private SSH key will be dumped to the current directory, make sure to save these to a safe place.
```
keypair_name="vulpos"
...
## Installation
The `vulpos-stack` tool needs to be installed on your local computer. The easiest way is to download a pre-built binary from the [Github Releases](https://github.com/reyu/vulpos-stack/releases) page. The other option is to [build from source](#build-from-source).

## Configuration
The vulpos-stack `config` subcommand should be run first to initialize a config file which will be stored in `$HOME/.vulpos.toml`. By default, an autogenerated stack name will be generated for `<vulpos-stackname>`; if you want to customize this name beware that the name must be globally unique in AWS or deployment will fail.
...
INFO[0000] Using config file: /Users/user/.vulpos.toml
...
[[custom-patches]]
  ...
  repo = "https://github.com/VulpOS/community_patches"
```

#### All Options
To see full list of options you can pass vulpos-stack you can use the help flag (-h). These flags can also be set as config values in the config file.

```none
...

Usage:
  vulpos-stack deploy [flags]
...
Global Flags:
      --config-file string   config file (default location to look for config is $HOME/.vulpos.toml)
```

## First Time Setup After Deployment
* Click on the email confirmation link sent to your email in order to start getting build notifications.
* You'll need to manually start your first build using `vulpos-stack` tool. Future builds will happen automatically based on the schedule defined in your configuration.

    ```sh 
    ./vulpos-stack build start
    ```
...
* After the build finishes, a factory image should be uploaded to the S3 release bucket that you can download. Be sure to replace the command below with your stack name and your device name (e.g. taimen).
    ...
    ```sh 
    aws s3 cp s3://<vulpos-stackname>-release/<device>-factory-latest.tar.xz .
    ```

* Use this factory image and [follow the instructions on flashing your device carefully](FLASHING.md).
* After successfully flashing your device, you will now be running VulpOS and all future updates will happen through the built in OTA updater.
* <b>I HIGHLY suggest backing up your generated signing keys and storing them somewhere safe</b>. To backup your signing keys:

    ```sh 
    aws s3 sync s3://<vulpos-stackname>-keys/ .
    # or if you are using encrypted keys
    aws s3 sync s3://<vulpos-stackname>-keys-encrypted/ .
    ```

## FAQ
### General
#### Should I use vulpos-stack?
Use this at your own risk.
#### Where can I get help, ask questions, keep up to date on development?
* For general questions and keeping up to date, use subreddit [/r/VulpOS](https://www.reddit.com/r/VulpOS/)
* If you run into any issues with vulpos-stack, please [file an issue or feature request on Github](https://github.com/reyu/vulpos-stack/issues) and provide all of the requested information in the issue template.
#### How do I update vulpos-stack?
Just download the new version of vulpos-stack and run deploy again (e.g. ./vulpos-stack deploy)
...
On build failure/success, the instance should terminate and upload its logs to S3 bucket called `<vulpos-stackname>-logs` and it's in a file called `<device>/<timestamp>`.
...
There is a flag you can pass `vulpos-stack` called `--prevent-shutdown` that will prevent the EC2 instance from terminating so that you can SSH into the instance and debug. Note that this will keep the instance online for 12 hours or until you manually terminate it.
...
* On your computer, start a VulpOS build with the `build` command. As part of the build process AttestationServer is deployed (should get more emails) and the Auditor app will be built and included into your resulting VulpOS build.
...
### Customizations
#### How do I customize VulpOS builds?
There are some advanced options that allow you to customize VulpOS builds to your liking by adding additional patches and prebuilt apps. These can only be setup in the config file and not through CLI flags.
...
##### Patches and Scripts
There is an option to execute patches and shell scripts against the AOSP build tree using `[[custom-patches]]` in the config file. This requires you provide a Git repo and a list of patches you want to apply during the build process. [There is a repo of useful patches that have been contributed by the community](https://github.com/VulpOS/community_patches) that are trusted and can be used here - or you could use your own if you wanted.
...
[[custom-scripts]]
  repo = "https://github.com/VulpOS/example_patch_shellscript"
...
[[custom-prebuilts]]
  modules = ["app1", "app2"]
  repo = "https://github.com/VulpOS/example_prebuilts"
```
...
```toml
# to add a remote line to manifest like this: <remote name="vulpos" fetch="https://github.com/VulpOS/" revision="master" />
[[custom-manifest-remotes]]
  name = "vulpos"
  fetch = "https://github.com/VulpOS/"
  revision = "master"

# to add a project line to manifest like this: <project path="packages/apps/Backup" name="backup" remote="vulpos" />
...
Some minimimum steps worth considering are having an account solely for building VulpOS with a strong password, enabling two factor authentication, enabling auditing with CloudTrail, and locking down access to your AWS API credentials.
#### What's the difference between the default option and encrypted signing keys option and what one should I use?
There are different configurations for VulpOS builds based on your threat model. Here's a breakdown of the two primary build configurations and how they compare:
* Using the standard VulpOS build process, your keys are autogenerated and stored in S3. This means your AWS account security become the most important part of maintaining secure signing keys. This default build option is a good fit for someone that is OK with putting some trust in AWS, wants hands off builds with no manual intervention, and doesn't want to deal with maintaining a passphrase for encrypting/decrypting signing keys. Even with this setup, this still means AWS has potential access to your signing keys. If your threat model included an attacker compromising your AWS account, then this would not suffice as they would be able to get access to your unencrypted signing keys.
...
* First you'll need to update your stack config file to use `encrypted-keys = true` and then run `vulpos-stack deploy` to update your stack. 
* Next you'll need to copy your existing signing keys from S3 bucket `<vulpos-stackname>-keys`, encrypt them with GPG using a strong passphrase, and then copy over encrypted keys to S3 encrypted keys bucket `<vulpos-stackname>-keys-encrypted`.
    ```sh
    ...
    aws s3 sync . s3://<vulpos-stackname>-keys-encrypted/ --exclude "*" --include "*.gpg"
    ```
* After running a full build and updating your device, you can remove the keys from the original `s3://<vulpos-stackname>-keys` bucket.
...
```sh
./vulpos-stack remove --name <vulpos-stackname> --region us-west-2
```
...
* [Liberapay](https://liberapay.com/vulpos/)
 ...
 * To compile from source you'll need to install Go (https://golang.org/) for your platform
  ```sh
  git clone github.com/reyu/vulpos-stack
  make tools
  make
  ```
